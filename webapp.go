...
/*linked from the home page. This page displays recent messages from all subscribed users*/
func subscription_feed(w http.ResponseWriter, r *http.Request) {
	if !is_valid_session(r) {
		log_out(w, r)
		return
	}
	page := `<!DOCTYPE html>
				...
				<body>
				    <h1>Recent Messages from Subscribed users</h1>
				    %s 
				</body>				
				</html>`

	//send a get sub feed request for recent 3 messages
	sub_feed_args := []string{SUB_FEED, my_user.uname, my_user.psw, "3"}
	sub_feed_response := query_server(sub_feed_args)
	is_success := (sub_feed_response[0] == SUCCESS_RESPONSE)
	if is_success {
		//print messages on web page
		messages := sub_feed_response[1]
		//put 2 newlines after every message
		messages = strings.Replace(messages, USER_MESSAGE_SEPERATOR, "<br /><br />", -1)
		fmt.Fprintf(w, page, messages)
	} else {
		fmt.Fprintf(w, "Failed to read messages. Server responded: %s.", sub_feed_response[1])
	}
}
func subscribe(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		//shouldnt be here
		fmt.Fprintf(w, "please try again from home page")
	case http.MethodPost:
		r.ParseForm()
		sub_uname := r.PostFormValue("uname")
		sub_args := []string{SUB, my_user.uname, my_user.psw, sub_uname}
		sub_response := query_server(sub_args)

		is_success := (sub_response[0] == SUCCESS_RESPONSE)
		if is_success {
			fmt.Fprintf(w, "successfully subscribed to %s.", sub_uname)
		} else {
			fmt.Fprintf(w, "Failed to subscribe. Server responded: %s.", sub_response[1])
		}
	}
}
func unsubscribe(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		r.ParseForm()
		sub_uname := r.PostFormValue("uname")
		unsub_args := []string{UNSUB, my_user.uname, my_user.psw, sub_uname}
		unsub_response := query_server(unsub_args)

		is_success := (unsub_response[0] == SUCCESS_RESPONSE)
		if is_success {
			fmt.Fprintf(w, "successfully un-subscribed to %s.", sub_uname)
		} else {
			fmt.Fprintf(w, "Failed to un-subscribe. Server responded: %s.", unsub_response[1])
		}
	}
}
...
func main() {
	...
	http.HandleFunc("/subscription_feed", subscription_feed)
